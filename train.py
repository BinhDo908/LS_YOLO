...
def train(hyp, opt, device, callbacks):  # hyp is path/to/hyp.yaml or hyp dictionary
    ...
    # Configure custom label directory when dataset uses non-standard img/label paths
    if 'label_dir' in data_dict:
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        if RANK in {-1, 0}:
            ...
            if (not nosave) or (final_epoch and not evolve):  # if save
                ...
                # save_pt = w/f'epoch{epoch}.pt'
                # torch.save(ckpt, save_pt)  # 保存每一轮权重
...
def parse_opt(known=False):
    ...
    parser.add_argument('--data', type=str, default= r'', help='dataset.yaml path')
